...
{
	if (houses[0].getPosInt(0) != size[0]/2)
	{
		Position hAz(xOnAzimut(houses[0].getPos()), yOnAzimut(houses[0].getPos())); 
		Position tpOnHouseLine((tp[1] - hAz[1] + hAz[0]*tan(pi - azimut) - tp[0]*1/tan(azimut))/(tan(pi - azimut) - 1/tan(azimut)), 1/tan(azimut)*((tp[1] - hAz[1] + hAz[0]*tan(pi - azimut) - tp[0]*1/tan(azimut))/(tan(pi - azimut) - 1/tan(azimut)) - tp[0]) + tp[0]);
		return Position(size[0]/2 + (calcDelta(hAz, tpOnHouseLine))*(calcDelta(houses[0].getPos(), hAz))/(size[0]/2 - houses[0].getPosInt(0)), houses[0].getPosInt(0));
	}
	if (houses[1].getPosInt(0) != size[0]/2)
	{
		Position hAz(xOnAzimut(houses[1].getPos()), yOnAzimut(houses[1].getPos())); 
		Position tpOnHouseLine((tp[1] - hAz[1] + hAz[0]*tan(pi - azimut) - tp[0]*1/tan(azimut))/(tan(pi - azimut) - 1/tan(azimut)), 1/tan(azimut)*((tp[1] - hAz[1] + hAz[0]*tan(pi - azimut) - tp[0]*1/tan(azimut))/(tan(pi - azimut) - 1/tan(azimut)) - tp[0]) + tp[0]);
		return Position(size[0]/2 + (calcDelta(hAz, tpOnHouseLine))*(calcDelta(houses[1].getPos(), hAz))/(size[0]/2 - houses[1].getPosInt(0)), houses[1].getPosInt(0));
	}
...
{
	return (myPos[1] - tp[1] + tp[0]*tan(pi - azimut) - myPos[0]*1/tan(azimut))/(tan(pi - azimut) - 1/tan(azimut));
}
double Photo::yOnAzimut(Position tp)
{
	return 1/tan(azimut)*(xOnAzimut(tp) - myPos[0]) + myPos[0];
} 
...
void Photo::show()
{
	...
	{
		houses[l].drawOnPhoto(*photo, l + 1);
